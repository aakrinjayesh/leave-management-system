...
// The actual Profile content, shared by both the routed /profile page (still
// used as the deep-link target for email/notification buttons) and the panel
// swapped into DashboardLayout's main content area when the topbar avatar is
// clicked - identical either way. `onClose`, when given, renders a close
// button in the header (the avatar-triggered panel case) - the routed page
// leaves it out since it already has the normal back-arrow for that.
export default function ProfileContent({ onClose }) {
  ...
  useEffect(() => {
    ...
    // Deferred a tick (not delayed - just off the synchronous effect body,
    // per react-hooks/set-state-in-effect) rather than moved into the
    // markAnniversaryCelebrationSeen().then() below, so the celebration still
    // plays immediately instead of waiting on that network round-trip.
    queueMicrotask(() => {
